...
function EMICalculator() {
  ...
  return (
    <div className="emi-calculator-container">
      <div className="header"><button className="back-button" onClick={() => navigate(-1)}>← Back</button></div>
      <main className="emi-calculator-content">
        ...
        <div className="input-group"><label>Type of Loan:</label><select value={loanType} onChange={(e) => selectLoanType(e.target.value)}>{Object.keys(loanRates).map((type) => <option key={type}>{type}</option>)}</select></div>
        <div className="input-group"><label>Loan Amount (₹):</label><input type="number" min="1" value={loanAmount} onChange={(e) => setLoanAmount(e.target.value)} placeholder="e.g. 500000" /></div>
        <div className="input-group"><label>Interest Rate (% p.a.):</label><input type="number" min="0.1" max="50" step="0.01" value={interestRate} onChange={(e) => setInterestRate(e.target.value)} /></div>
        <div className="input-group"><label>Loan Tenure (Months):</label><input type="number" min="1" max="360" value={loanTenure} onChange={(e) => setLoanTenure(e.target.value)} /></div>
        <button className="calculate-button" onClick={calculateEMI}>Calculate EMI</button>

        {summary && <div className="emi-summary">
          <div><span>Monthly EMI</span><strong>{formatCurrency(summary.emi)}</strong></div>
          <div><span>Total Interest</span><strong>{formatCurrency(summary.totalInterest)}</strong></div>
          <div><span>Total Payment</span><strong>{formatCurrency(summary.totalPayment)}</strong></div>
        </div>}

        {emiDetails.length > 0 && <div className="emi-table-container">
          <h3>Amortization Schedule</h3>
          <div className="table-scroll"><table>
            <thead><tr><th>Month</th><th>EMI (₹)</th><th>Principal (₹)</th><th>Interest (₹)</th><th>Balance (₹)</th></tr></thead>
            <tbody>{emiDetails.map((row) => <tr key={row.month}><td>{row.month}</td><td>{row.emi.toFixed(2)}</td><td>{row.principalPaid.toFixed(2)}</td><td>{row.interestPaid.toFixed(2)}</td><td>{row.remainingPrincipal.toFixed(2)}</td></tr>)}</tbody>
          </table></div>
        </div>}
      </main>
